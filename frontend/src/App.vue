...
import { completeEnrollment, createCourse, createEnrollment, createStudent, getEligibility } from './api'
...
async function confirmAnswer() {
  ...
  const grade = wasCorrect.value ? 8.5 : 4.5
  ...
  try {
    ...
    isEligibleForExtraCourses.value = eligibility.eligibleForExtraCourses

    confettiPieces.value = wasCorrect.value ? buildConfetti() : []
  } catch (error) {
    submitError.value = error.message || 'Nao foi possivel falar com a API.'
    answered.value = false
  } finally {
        ...
        <div v-if="!course.completed" key="pending" class="stage stage-locked">
          ...
          <Transition name="fade-slide" mode="out-in">
            ...
            <div v-else key="quiz" class="quiz-card">
              ...
              <p v-if="submitError" class="quiz-error">{{ submitError }}</p>

              <button
                class="btn-primary btn-block"
                :disabled="selectedOption === null || answered || submitting"
                @click="confirmAnswer"
              >
